...
class DuckDuckGoJobBoardCrawler:
    ...
    def __init__(
        ...
        playwright_proxy_mode: str = "none",
        proxy_stats_file: Optional[str] = None,
    ):
        ...
        # Playwright proxy behavior: 'none' | 'single' | 'rotate'
        self.playwright_proxy_mode = (playwright_proxy_mode or "none").lower()
        # proxy stats for health reporting
        self.proxy_stats = {p: {"attempts": 0, "successes": 0, "failures": 0, "banned_until": None} for p in self.proxies}
        self.proxy_stats_file = proxy_stats_file
    ...
    def _ban_proxy(self, proxy_mapping: Dict[str, str]) -> None:
        ...
        try:
            ...
            # update stats
            st = self.proxy_stats.get(url) if isinstance(self.proxy_stats, dict) else None
            if st is not None:
                st["failures"] = st.get("failures", 0) + 1
                st["banned_until"] = self._proxy_blacklist[url]
            logger.warning("banned proxy %s for %ds", url, self.proxy_ban_seconds)
            # persist stats
            try:
                self.save_proxy_stats()
            except Exception:
                pass
        except Exception:
            pass

    def save_proxy_stats(self, path: Optional[str] = None) -> None:
        """Persist proxy stats to JSON file for external inspection."""
        try:
            p = path or self.proxy_stats_file
            if not p:
                return
            import json
    ...
    def _fetch_with_playwright(self, base_url: str, params: Dict[str, str]) -> Optional[str]:
        ...
        # choose proxy for playwright based on configured mode
        play_proxy = None
        if self.playwright_proxy_mode == "single":
            if self.proxies:
                play_proxy = self.proxies[0]
        elif self.playwright_proxy_mode == "rotate":
            if self.proxies:
                next_p = self._get_next_proxy()
                if next_p:
                    # next_p is mapping {'http': url, 'https': url}
                    play_proxy = next_p.get("http") or next_p.get("https")

        def _parse_proxy_url(purl: str):
            """Parse proxy URL into Playwright proxy dict {server, username, password}"""
            try:
        ...
        proxy_option = None
        if play_proxy:
            proxy_option = _parse_proxy_url(play_proxy)
        ...
        # record stats for proxy if used
        if play_proxy:
            try:
                st = self.proxy_stats.get(play_proxy)
                if st is None:
                    self.proxy_stats[play_proxy] = {"attempts": 1, "successes": 1, "failures": 0, "banned_until": None}
                else:
